...
var EmpDBDirectives = angular.module('EmpDB.EmpDBDirectives',[]);
EmpDBDirectives.directive('numbersOnly', function () {
    return {
        require: 'ngModel',
...
EmpDBDirectives.directive('numbersAndDotOnly', function () {
    return {
        require: 'ngModel',
        link: function (scope, element, attr, ngModelCtrl) {
            function fromUser(text) {
...
EmpDBDirectives.factory('convertMillToDate', function($filter) {
    return {
        toDate: function(mill) {
           if(!angular.isUndefined(mill) && mill !==null && mill != undefined ){
        	   var date = new Date(mill)
        	   var utc=new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()+1,  date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds());
        	   return utc;
            }
        }
    };
...
EmpDBDirectives.directive('phoneValid', function () {
    return {
        require: 'ngModel',
        link: function (scope, element, attr, ngModelCtrl) {
            function fromUser(text) {
...
EmpDBDirectives.directive('fcsaNumber', function () {
